...
const ReceivedInvitesList = ({ setLoad, load }: Props) => {
  ...
  const fetchReceived = async ({ pageParam = 1, status }: any): Promise<Page> => {
    const response = await fetch(
      `/api/financial-goals/received-invitations?page=${pageParam}&size=10&status=${status}`,
      {
        method: "GET",
        credentials: "include",
  ...
  const {
    ...
    queryKey: ["received-invites", receivedFilter],
  ...
  const handleAccept = async (id: string) => {
    await onAcceptInvite(id, "financial-goals", () => {
      setLoad(true);
      refetch();
    });
  };
  ...
  const handleReject = async (id: string) => {
    await onRevokeInvite(id, "financial-goals", () => {
      setLoad(true);
      refetch();
    });
  };
