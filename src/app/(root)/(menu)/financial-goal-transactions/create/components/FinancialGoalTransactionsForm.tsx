...
import { useFinancialGoalTransactionForm } from "../hooks/useFinancialGoalTransactionForm";
...
const transactionTypes = [
  { id: "contribution", label: "Contribution", description: "Add funds to your goal" },
  { id: "withdrawal", label: "Withdrawal", description: "Remove funds from your goal" },
];
...
export function FinancialGoalTransactionsForm() {
  ...
  const {
    ...
    financialGoals,
    accounts,
  } = useFinancialGoalTransactionForm();

  const selectedGoalData = financialGoals.find((g) => g.id === formData.financial_goal_id);
  const progress = selectedGoalData
    ? Math.round((selectedGoalData.current / selectedGoalData.target) * 100)
    : 0;
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      <div className="space-y-3">
        ...
        <Select
          ...
          onValueChange={(e: string) => {
            setFormData((prev: any) => ({ ...prev, financial_goal_id: e }));
          }}
        >
          ...
          <SelectContent className="bg-card border-border">
            {!loadingGoals &&
              financialGoals.map((goal: FinancialGoalBasic) => (
                <SelectItem
                  key={goal.id}
                  value={goal.id}
                ...
                >
                  <div className="flex flex-col items-start gap-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {goal.currentFormated} of {goal.targetFormated}
                    </span>
                  </div>
                </SelectItem>
              ))}
        ...
        {selectedGoalData && (
          <div className="p-4 rounded-xl bg-secondary border border-border space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-foreground">{selectedGoalData.name}</span>
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>{selectedGoalData.currentFormated}</span>
              <span>{selectedGoalData.targetFormated}</span>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {transactionTypes.map((type) => (
            <button
              ...
              type="button"
              onClick={() => {
                setFormData((prev: any) => ({ ...prev, type: type.id }));
              }}
              className={cn(
            ...
            >
              ...
              <div className="text-xs text-muted-foreground mt-1.5">{type.description}</div>
              {formData.type === type.id && (
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground text-lg">
                {selectedGoalData?.currencySymbol || "$"}
              </span>
              <Input
                id="amount"
                type="number"
      ...
      <SubmitFormButton
        ...
        isDisable={
          !formData.financial_goal_id || !formData.type || !formData.account_id || !formData.amount
        }
      />

      {/* Form Helper */}
      <p className="text-center text-xs text-muted-foreground">
        All transactions are securely processed and recorded instantly.
      </p>
    </form>
  );
}

export default FinancialGoalTransactionsForm;
